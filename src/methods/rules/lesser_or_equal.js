export function __validate_lesser_or_equal({ruleObj, fieldName, fieldLabel, index, hasNullableRule}) {
    ...
    if (isNaN(target)) {
        return {
            isValid: false,
            message: `Target value must be a number for lesser or equal numbers match for [${fieldName}]`
        };
    }
    ...
    if (fieldValue > target) {
        ...
        isValid = false;
        message = ruleObj.message ? ruleObj.message : `${fieldLabel} must be lesser or equal to ${target}`;
    }
