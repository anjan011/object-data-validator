import {parse, isAfter, isBefore, format, subHours} from 'date-fns';
import {isValid as isDateValid} from 'date-fns/isValid';
import {DEFAULT_DATE_FORMAT} from "../../constants";

export function __validate_date_string_after({ruleObj, fieldName, fieldLabel, index, hasNullableRule}) {
    ...
    if (typeof fieldValue === 'string' && fieldValue.trim() !== '') {
        ...
        if (isDateValid(fieldDate)) {
            ...
            if(!inclusive) {
                ...
                if(!isAfter(fieldDate, dateToCompare)) {
                    isValid = false;
                    message = ruleObj.message ?
                        ruleObj.message :
                        (
                            DEFAULT_DATE_FORMAT === dateFormat ?
                                `${fieldLabel} must be a valid date string after ${format(dateToCompare, dateFormat)}` :
                                `${fieldLabel} must be a valid date string after ${format(dateToCompare, dateFormat)} in this format: ${dateFormat}`
                        );
                }

            } else {
                if(!isAfter(fieldDate, subHours(dateToCompare,24))) {
                    isValid = false;
                    message = ruleObj.message ?
                        ruleObj.message :
                        (
                            DEFAULT_DATE_FORMAT === dateFormat ?
                                `${fieldLabel} must be a valid date string after or equal to ${format(dateToCompare, dateFormat)}` :
                                `${fieldLabel} must be a valid date string after or equal to ${format(dateToCompare, dateFormat)} in this format: ${dateFormat}`
                        );
                }
            }
