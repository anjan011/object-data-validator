export function __validate_greater_or_equal({ruleObj, fieldName, fieldLabel, index, hasNullableRule}) {
    ...
    if (isNaN(target)) {
        return {
            isValid: false,
            message: `Target value must be a number for greater or equal numbers match for [${fieldName}]`
        };
    }
    ...
    if (fieldValue < target) {
        ...
        isValid = false;
        message = ruleObj.message ? ruleObj.message : `${fieldLabel} must be greater or equal to ${target}`;
    }
