export function __validate_min_length({ruleObj, fieldName, fieldLabel,index, hasNullableRule}) {
    ...
    if (typeof fieldValue !== 'string') {
        return {
            isValid: false,
            message: `${fieldLabel} must be a string to assert minimum length rule`,
            fieldValue
        }
    }
    ...
    if(this.isNullOrUndefined(length)) {
        return {
            isValid: false,
            message : `Target length is not provided for minimum length match for [${fieldName}]`
        };
    }
    ...
    if (length < 1) {
        return {
            isValid: false,
            message: `Target length must be at least 1 for minimum length match for [${fieldName}]`
        };
    }
    ...
    if(fieldValue.length < length) {
        ...
        isValid = false;
        message = ruleObj.message ? ruleObj.message : `${fieldLabel} must be at least ${length} character${length > 1 ? 's' : ''} long`;
    }
