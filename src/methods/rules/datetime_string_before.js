...
export function __validate_datetime_string_before({ruleObj, fieldName, fieldLabel, index, hasNullableRule}) {
    ...
    if(this._isEmptyString(date)) {
        this.ruleError({
            ...
            error: `Target date is required for datetime_string_before rule`
        });
    }
    ...
    if (!isAfter(dateToCompare,fieldDate)) {
        ...
        isValid = false;
        message = ruleObj.message ? ruleObj.message : `${fieldLabel} must be a valid date string before ${date} in this format: ${dateFormat}`;
    }
